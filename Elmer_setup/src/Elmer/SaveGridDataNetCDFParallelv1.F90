...
!    USE AscBinOutputUtils
    ...
    LOGICAL :: Found,SinglePrec,NoFileIndex,Visited = .FALSE.
  ...
  CONTAINS
    ...
    SUBROUTINE WriteNetCDFFile( NetCDFFile )
      ...
      INTEGER :: i,j,k,l,dofs,Rank,cumn,n,vari,sdofs,ind,IsVector,IsAppend,&
                 GridPoints,Offset,NumVars,DimLen,NumVars2,AllInd,FieldLength,&
      ...
      !All NetCDF calls walled off to all except boss partion
      !ALLREDUCE for ind to work out if any partition returned non-0
      !Send value to boss partition to write (for 0 and non-0)
      ...
      !Everything below here boss partition only? No, down to write values.
      !Set up dims here beforehand
      IF(Part == 0 .OR. .NOT. Parallel) THEN
        IF(Dim==2) DimId(3) = 0
        DO i=1,Dim
          IF(i==1) THEN
      ...
      !Create coordinate variables so you know where the grid came from      
      NumVars = 1
      ...
      IF(Part == 0 .OR. .NOT. Parallel) THEN
        ...
        !Create all the variables in the NetCDF. Vectors will have an unknown
        !number of components, so just loop over them until found them all  
        DO i=1,2
            ...
            IF(i==1) THEN          
              ...
              IF(Dim==2) THEN
                NetCDFStatus = NF90_DEF_VAR(FileId, TRIM(FieldName), 6,&
                             (/ DimId(1), DimId(2) /),VarId(NumVars))
                IF ( NetCDFStatus /= 0 ) THEN
                  CALL Fatal( 'WriteNetCDFFile', 'NetCDF variable could not be created: '//TRIM(FieldName))
                END IF
              ELSE IF(Dim==3) THEN
                NetCDFStatus = NF90_DEF_VAR(FileId, TRIM(FieldName), 6,&
                             (/ DimId(1), DimId(2), DimId(3) /),VarId(NumVars))
                IF ( NetCDFStatus /= 0 ) THEN
                CALL Fatal( 'WriteNetCDFFile', 'NetCDF variable could not be created: '//TRIM(FieldName))
                END IF
              END IF
      ...
      !Add coordinate values first
      IF(Part == 0 .OR. .NOT. Parallel) THEN
        DO j=1,Dim
          DO i=1,INT((GridExtent(2*j)-GridExtent((2*j)-1))+1)
            val = GridExtent((2*j)-1) + (GridDx(j) * (i-1)) + GridOrigin(j)
      ...
      !Send NumVars and NumVars2 from boss to all partitions. Also send FileId
      !and VarId, so all partitions can read variables (this would be much
      !easier if I could just broadcast the FieldName, but MPI isn't good with
      !strings)
      IF(Parallel) THEN
        ...
        !CALL MPI_BCAST(FileId, 1, MPI_INTEGER, 0, ELMER_COMM_WORLD, ierr)
        !CALL MPI_BCAST(VarId, 100, MPI_INTEGER, 0, ELMER_COMM_WORLD, ierr)
      END IF      
                ...
                IF( ind == 0 ) THEN
                  IF(Part == 0 .OR. .NOT. Parallel) THEN
                    IF( SinglePrec ) THEN
                      fvalue = 0.0
                      IF(Dim == 2) THEN
                        NetCDFStatus = NF90_PUT_VAR(FileId, VarId(NumVars2), fvalue, start=(/ i,j /))
                      ELSE IF(Dim == 3) THEN
                        NetCDFStatus = NF90_PUT_VAR(FileId, VarId(NumVars2), fvalue, start=(/ i,j,k /))
                      END IF
                      IF ( NetCDFStatus /= 0 ) THEN
                        CALL Fatal( 'WriteNetCDFFile', 'NetCDF variable could not be written: '//TRIM(FieldName))
                      END IF
                    ELSE
                      val = 0.0_dp
                      IF(Dim == 2) THEN
                        NetCDFStatus = NF90_PUT_VAR(FileId, VarId(NumVars2), val, start=(/ i,j /))
                      ELSE IF(Dim == 3) THEN
                        NetCDFStatus = NF90_PUT_VAR(FileId, VarId(NumVars2), val, start=(/ i,j,k /))
                      END IF
                      IF ( NetCDFStatus /= 0 ) THEN
                        CALL Fatal( 'WriteNetCDFFile', 'NetCDF variable could not be written: '//TRIM(FieldName))
                      END IF
