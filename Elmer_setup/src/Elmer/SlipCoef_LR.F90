FUNCTION SlipCoef(Model,nodenumber,VarIn)  RESULT(VarOut)
 ...
 FUNCTION CalculSlc_LS_LinearRegression (model, nodenumber, VarIn) RESULT(VarOut)
  ...
  IMPLICIT NONE 
  TYPE(Model_t) :: model
  REAL (KIND=dp) :: VarIn(8)    ! alpha_anom, c1, c2, c3, c4, b_lim, bed, distance, multiple
  REAL (KIND=dp) :: VarOut      ! slc
  INTEGER :: nodenumber
  ...
  alpha=VarIn(1)+(VarIn(2)*VarIn(7)+VarIn(6))*10**(VarIn(4)*VarIn(7)+VarIn(5)*VarIn(8))

  VarOut=VarIn(6)*10**alpha
   
END FUNCTION CalculSlc_LS_LinearRegression

FUNCTION CalculSlc_LS_simple (model, nodenumber, VarIn) RESULT(VarOut)
  USE DefUtils
  IMPLICIT NONE 
  TYPE(Model_t) :: model
  REAL (KIND=dp) :: VarIn(3)    ! slc_anom, distance, multiple
  REAL (KIND=dp) :: VarOut      ! slc
  INTEGER :: nodenumber
  
  TYPE(ValueList_t), POINTER :: constants
  REAL(kind=dp) :: slc
  REAL(kind=dp) :: slope,max_ls

   ! inquire SSA friction exponent from Material properties
  constants => GetConstants()
  IF (.NOT. ASSOCIATED(constants)) THEN
    CALL Fatal('CalculSlc_LS', "No constants found?")
  ENDIF
  slope = ListGetConstReal( Constants, 'slope',UnFoundFatal=.TRUE.)  
  max_ls = ListGetConstReal( Constants, 'max_ls',UnFoundFatal=.TRUE.)  

  ! alpha = alpha_anom + (c2*bed-b_lim)*10**(c3*bed)*(10**(c4*ls))
  slc=VarIn(1)+slope*VarIn(2)/((VarIn(2)/max_ls)+1)
...
END FUNCTION CalculSlc_LS_simple

FUNCTION CalculSlc_haf_simple (model, nodenumber, VarIn) RESULT(VarOut)
  USE DefUtils
  IMPLICIT NONE 
  TYPE(Model_t) :: model
  REAL (KIND=dp) :: VarIn(4)    ! slc_anom, H, bedrock, multiple
  REAL (KIND=dp) :: VarOut      ! slc
  INTEGER :: nodenumber
  
  TYPE(ValueList_t), POINTER :: constants
  REAL(kind=dp) :: slc,haf
  REAL(kind=dp) :: slope,max_haf, rhow, rhoi
  ...
  ENDIF
  slope = ListGetConstReal( Constants, 'slope',UnFoundFatal=.TRUE.)  
  max_haf = ListGetConstReal( Constants, 'max_haf',UnFoundFatal=.TRUE.)  
  rhow = ListGetConstReal( Constants, 'rhow',UnFoundFatal=.TRUE.)  
  rhoi = ListGetConstReal( Constants, 'rhoi',UnFoundFatal=.TRUE.)  

  haf=VarIn(2)+(rhow/rhoi)*VarIn(3)

  ! alpha = alpha_anom + (c2*bed-b_lim)*10**(c3*bed)*(10**(c4*ls))
  slc=VarIn(1)+slope*haf/((haf/max_haf)+1)

  VarOut=VarIn(4)*slc
   
END FUNCTION CalculSlc_haf_simple

FUNCTION CalculSlc_haf_J (model, nodenumber, VarIn) RESULT(VarOut)
  USE DefUtils
  IMPLICIT NONE 
  TYPE(Model_t) :: model
  REAL (KIND=dp) :: VarIn(4)    ! slc_anom, H, bedrock, multiple
...
END FUNCTION CalculSlc_haf_J

FUNCTION CalculSlc_SlcMultiple (model, nodenumber, VarIn) RESULT(VarOut)
  USE DefUtils
  IMPLICIT NONE 
  TYPE(Model_t) :: model
  REAL (KIND=dp) :: VarIn(2)    ! slc0, multiple
  REAL (KIND=dp) :: VarOut      ! slc
  INTEGER :: nodenumber
  
  VarOut=VarIn(2)*VarIn(1)
   
END FUNCTION CalculSlc_SlcMultiple